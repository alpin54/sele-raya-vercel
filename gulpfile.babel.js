...
const DEV_PATH = {
  ...
  style: [`${SRC}/_core/styles/**/*.scss`, `${SRC}/components/**/*.scss`, `${SRC}/templates/**/*.scss`],
  script: {
    vendor: {
      order: [
        `${SRC}/_core/scripts/vendors/jquery-3.6.0.min.js`,
        `${SRC}/_core/scripts/vendors/owl.carousel.min.js`,
        `${SRC}/_core/scripts/vendors/*.js`
      ],
      concat: 'vendor.js'
    },
    watch: [`${SRC}/core/scripts/**/*.js`, `${SRC}/components/**/*.js`, `${SRC}/templates/**/*.js`],
    main: `${SRC}/_core/scripts/${fileName}.js`
  },
  image: `${SRC}/assets/images/**/*`,
  video: `${SRC}/assets/videos/**/*`,
  fonts: `${SRC}/assets/fonts/**/*`,
  others: {
    forbidden: `${SRC}/assets/others/html/403.html`,
    data: `${SRC}/assets/others/data/*.json`,
    customStyle: `${SRC}/assets/others/css/*.css`,
    customScript: `${SRC}/assets/others/js/*.js`
  }
};
  ...
  return src(DEV_PATH.image)
    ...
    .pipe(
      imagemin([
        ...
        // svg
        imagemin.svgo({
          plugins: [{ removeViewBox: true }, { cleanupIDs: false }, { removeUnknownsAndDefaults: false }]
        }),
        // gif
        imageminGiflossy({
          optimizationLevel: 3,
...
/* ------------------------------------------------------------------------------
@name: copyVideo
@description: Copy video files to production directory
--------------------------------------------------------------------------------- */
export const copyVideo = () => {
  return src(DEV_PATH.video)
    .pipe(changed(BUILD_PATH.video))
    .pipe(dest(BUILD_PATH.video))
    .pipe(server.stream && server.stream()) // Only if browserSync is running
    .on('end', () => {
      console.log(c[COLORS.success].bold('--------- Video files finished copying! ---------'));
    });
};
...
export const copyOthers = parallel(copyData, copyForbidden, copyCustomStyle, copyCustomScript);
...
export const devWatch = (done) => {
  ...
  // VENDOR JS
  watch(DEV_PATH.script.vendor.order, series(copyVendorJS, reload));
  ...
  // Video
  watch(DEV_PATH.video, copyVideo);
...
exports.default = series(
  compressImage,
  parallel(copyVendorJS, copyFonts, copyVideo),
...
/* ------------------------------------------------------------------------------
@name: Start Task
@description: Start Task will be run the project
--------------------------------------------------------------------------------- */
